---
name: crustify-orchestrator
description: >-
  The orchestrator agent's playbook for driving crustify, in two phases. Setup:
  toolchain install through the first commit of the scaffolded Rust tree —
  authoring build.json, cli-config.json and a target's scope-config.json,
  building the CodeQL database and extracting the T1/T2 tables, crate
  placement, scaffold and bindgen. Translation: planning, running, landing and
  auditing waves of translate agents over that tree. Read Setup before any
  wave; every later stage reads what it produces.
---
