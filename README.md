...
**Batch Scheduling.** `crustify-oracle schedule` composes an objective-neutral
wave document whose sequential steps contain deterministic batches.
`crustify-cli translate` routes them through the type or
...
| `translate` | executes a wave document, step by step, one agent per batch | `<wave.json>` `--objective` `--dry-run` · `--model` `--billing` `--parallel-max` `--override-base-prompt` `--no-override-base-prompt` `--no-console` `--no-file-log` |
...
`--objective wrap\|port\|review` says what to do with the submitted wave. Raw
lifetime waves are generated by the oracle with
`schedule --lifetime-for`. Set
`--parallel-max 1` for serial execution; larger values cap concurrent batches
inside each topological step. Start with `--dry-run`.
