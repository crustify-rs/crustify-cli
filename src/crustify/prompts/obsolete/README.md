...
| `port.md` | `CrustifyPort` — the port-stage codegen agent: translated port-scope symbols to native Rust, calling the safe wrap API for FFI dependencies | Stage not implemented. `crustify port` and its flags still parse so an existing driver keeps working, but nothing is emitted. |
